...
def add_prefix(state_dict):
    """Add prefix to certain part of the keys in the state_dic
       Each prefix is repeated 42 times for DINO small
    """
...
#     # # Uncomment this if you want to implement original weights loading
#     # # remove `module.` prefix
#     # state_dict = {k.replace("module.", ""): v for k, v in state_dict.items()}
#     # # remove `backbone.` prefix induced by multicrop wrapper
#     # state_dict = {k.replace("backbone.", ""): v for k, v in state_dict.items()}

#     # add `teacher.backbone.` prefix
#     teacher_state_dict = {f"teacher.backbone.{k}": v for k, v in state_dict.items() if "pos_embed" not in k.split(".")}
#     # add `prefixes` to the keys
#     teacher_state_dict = add_prefix(teacher_state_dict)

#     msg = model.load_state_dict(teacher_state_dict, strict=False)
