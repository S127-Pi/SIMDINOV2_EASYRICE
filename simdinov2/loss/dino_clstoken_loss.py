...
class MCRLoss(DINOCenter):
    ...
    def forward(self, student_feat_list, teacher_feat_list, no_diag=True, normalized=True):
        ...
        comp_loss, global_comp_loss = self.calc_compression(student_feat, teacher_feat, no_diag=no_diag) # cosine similarity loss between teacher and student
    ...
    def calc_compression(self, student_feat_list, teacher_feat_list, no_diag=True):
        ...
        comp_loss = 0
        sim = (teacher_feat_list.unsqueeze(1)*student_feat_list.unsqueeze(0)).sum(-1).mean(-1) # Teacher: 2x1xNxD  Student: 1x10xNxD
